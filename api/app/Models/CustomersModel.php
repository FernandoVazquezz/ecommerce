<?php
    ...
    class CustomersModel extends Models{
        public function selectCustomers(){
            ...
            $cons = $this->db->pdo->prepare('SELECT * FROM customers');
            $cons->execute();
        ...
        public function insertCustomers($customer){
            ...
            $result->bindParam(':phone', $customer['phone'], \PDO::PARAM_INT);
            $result->bindParam(':addressLine1', $customer['addressLine1'], \PDO::PARAM_INT);
            $result->bindParam(':addressLine2', $customer['addressLine2'], \PDO::PARAM_INT);
            $result->bindParam(':city', $customer['city'], \PDO::PARAM_STR);
            $result->bindParam(':state', $customer['state'], \PDO::PARAM_STR);
            $result->bindParam(':postalCode', $customer['postalCode'], \PDO::PARAM_INT);
            ...
            $result->bindParam(':creditLimit', $customer['creditLimit'], \PDO::PARAM_INT);
            $result->execute();
            if(!is_null($this->db->error()[1])){
                return array(
                    'success' => false,
